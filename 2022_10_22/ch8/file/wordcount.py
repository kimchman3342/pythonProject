...
# 각 단어의 빈도수를 구해주는 함수        * 들여쓰기 간격이 일정한 경우에만 코드가 연결 됨.
def getTextFreq(filename):
    with open(filename, 'r', encoding='utf-8') as f:
        text = f.read()          # data.txt 파일 전체 내용을 읽어옴
        tmp = text.split()       # split()함수로 파싱된(분리된) 단어들을 리스트로 리턴     * split("")와 동일
...
print(sorted(result.items(), key=lambda x : x[0], reverse=True))    # reverse=True 내림차순
...
for c, freq in result:
    print('[%s] - [%d]회' %(c, freq))       # [%s] = 문자데이터, [%d] = 숫자데이터