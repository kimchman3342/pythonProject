...
형식 :  변수[ 시작 인덱스 번호 : 끝 인덱스 번호 ]    *'range 함수'랑 문자열 순서 동일.
'''
...
# 날짜와 날씨를 슬라이싱으로 분리
str = '20201024Rainy'
date = str[:8]              # 20201024 (날짜 슬라이싱) *변수 = str[ :출력숫자 + 1]
weather = str[8:]           # Rainy    (날씨 슬라이싱) *변수 = str[정확한 숫자: ]
print(date,':', weather)    # 분리하여 가운데 콜론(:)넣고 출력

# str 변수의 년, 월, 일을 슬라이싱 * 위와 동일한 원리
year = str[:4]              # 2020
month = str[4:6]            # 10
day = str[6:8]              # 24
